...
Node::Node (ORB_SLAM2::System::eSensor sensor, ros::NodeHandle &node_handle, image_transport::ImageTransport &image_transport) {
  ...
  node_handle_.param(name_of_node_ + "/bUseViewer", bUseViewer_, false);

  orb_slam_ = new ORB_SLAM2::System (voc_file_name_param_, settings_file_name_param_, sensor, map_file_name_param_, load_map_param_, bUseViewer_);

  service_server_ = node_handle_.advertiseService(name_of_node_+"/save_map", &Node::SaveMapSrv, this);//地图服务
...
vector<float> Node::TransformFromMat (cv::Mat position_mat,bool vec) {
  cv::Mat rotation(3,3,CV_32F);
  cv::Mat translation(3,1,CV_32F);
  ...
  for(int i=0;i<3;i++)
  {
    pos.push_back(tf_camera_translation[i]);
  }
  return pos;
}
...
void Node::ParamsChangedCallback(orb_slam2_ros::dynamic_reconfigureConfig &config, uint32_t level) {//动态参数
  orb_slam_->EnableLocalizationOnly (config.localize_only);//定位模式
  min_observations_per_point_ = config.min_observations_for_ros_map;

  if (config.reset_map) {//地图重设
    orb_slam_->Reset();
    config.reset_map = false;
  }

  orb_slam_->SetMinimumKeyFrames (config.min_num_kf_in_map);//最小关键帧
}


bool Node::SaveMapSrv (orb_slam2_ros::SaveMap::Request &req, orb_slam2_ros::SaveMap::Response &res) {//地图存储服务
